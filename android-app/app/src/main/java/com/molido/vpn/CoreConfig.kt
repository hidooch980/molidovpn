...
object CoreConfig {
     ...
     * absent. See [MolidoVpnService.raiseOuterLeg].
     *
     * Written only after the byte threshold in
     * [MolidoVpnService.recordWorkingPlainTransport] — a handshake is not
     ...
     * [MolidoVpnService.armRegionPhase]. A plain Psiphon connect always takes
     ...
     * the dead `if (!psiphonVpnMode)` arm in `MolidoVpnService.onConnected` is what
    ...
    fun portRejection(port: Int): String? = when {
        ...
        port in RESERVED_PORTS -> "$port is used internally by MolidoVPN"
        else -> null
    }
