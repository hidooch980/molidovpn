...
object AutoConnect {
    ...
    fun enabled(context: Context): Boolean =
        context.getSharedPreferences(MolidoTileService.SETTINGS, Context.MODE_PRIVATE)
    ...
    fun configJson(context: Context): String {
        val prefs = context.getSharedPreferences(MolidoTileService.SETTINGS, Context.MODE_PRIVATE)
        val armed = prefs.getBoolean(
            MolidoTileService.CHAIN_ARMED,
            MolidoTileService.CHAIN_ARMED_DEFAULT,
        )
        val picked = prefs.getString(
            MolidoTileService.DEFAULT_PROTOCOL,
            MolidoTileService.Companion.Protocol.AUTO.coreName,
        )
        return if (armed && picked == MolidoTileService.Companion.Protocol.PSIPHON.coreName) {
            CoreConfig.json(context, MolidoVpnService.CHAIN_PROTOCOL_MARKER.lowercase())
        } else {
    ...
    fun connect(context: Context): Boolean {
        ...
        context.startForegroundService(
            Intent(context, MolidoVpnService::class.java)
                .setAction(MolidoVpnService.ACTION_CONNECT)
                .putExtra(MolidoVpnService.EXTRA_CONFIG, configJson(context))
        )
        return true
    }
}
