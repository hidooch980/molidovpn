...
class MolidoWidget : AppWidgetProvider() {
    ...
    companion object {
        const val ACTION_TOGGLE = "com.molido.vpn.WIDGET_TOGGLE"
        ...
        private fun currentStatus(context: Context): String {
            val saved = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
                .getString(KEY_STATUS, MolidoVpnService.STATUS_DISCONNECTED)
                ?: MolidoVpnService.STATUS_DISCONNECTED
            // A stale "connected" after the process died must not stick.
            if (saved == MolidoVpnService.STATUS_CONNECTED && !TunnelStatus.isActive()) {
                return MolidoVpnService.STATUS_DISCONNECTED
            }
            return saved
        }
        ...
        fun render(context: Context) {
            ...
            val connected = status == MolidoVpnService.STATUS_CONNECTED
            val busy = status == MolidoVpnService.STATUS_CONNECTING ||
                status == MolidoVpnService.STATUS_STARTING ||
                status == MolidoVpnService.STATUS_SCANNING
...
class WidgetActionReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        val app = context.applicationContext
        when (intent.action) {
            MolidoVpnService.ACTION_STATUS -> {
                // Traffic/exit-IP broadcasts share the action but carry no status.
                val status = intent.getStringExtra(MolidoVpnService.EXTRA_STATUS) ?: return
                MolidoWidget.saveStatus(app, status)
                MolidoWidget.render(app)
            }
            MolidoWidget.ACTION_TOGGLE -> {
                try {
                    if (TunnelStatus.isActive() || MolidoVpnService.STATUS_CONNECTING ==
                        ...
                        MolidoWidget.saveStatus(app, MolidoVpnService.STATUS_DISCONNECTED)
                    } else if (AutoConnect.connect(app)) {
                        MolidoWidget.saveStatus(app, MolidoVpnService.STATUS_CONNECTING)
                    }
                } catch (e: Exception) {
