...
class MainActivity : Activity() {
    ...
    private val statusReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            ...
            intent.getStringExtra(MolidoVpnService.EXTRA_EXIT_IP)?.let { ip ->
                if (ip.isNotBlank()) {
                    coreExitIp = ip
                    coreExitCountry = ""
            ...
            if (intent.hasExtra(MolidoVpnService.EXTRA_TRAFFIC_TX)) {
                trafficTx = intent.getLongExtra(MolidoVpnService.EXTRA_TRAFFIC_TX, 0)
                trafficRx = intent.getLongExtra(MolidoVpnService.EXTRA_TRAFFIC_RX, 0)
                trafficSpeedTx = intent.getLongExtra(MolidoVpnService.EXTRA_TRAFFIC_SPEED_TX, 0)
                trafficSpeedRx = intent.getLongExtra(MolidoVpnService.EXTRA_TRAFFIC_SPEED_RX, 0)
                trafficMonthTx = intent.getLongExtra(MolidoVpnService.EXTRA_TRAFFIC_MONTH_TX, 0)
                trafficMonthRx = intent.getLongExtra(MolidoVpnService.EXTRA_TRAFFIC_MONTH_RX, 0)
                renderTrafficMonitor()
                renderHomeMetrics()
                return
            }
            when (intent.getStringExtra(MolidoVpnService.EXTRA_STATUS)) {
                MolidoVpnService.STATUS_CONNECTING -> showConnecting(
                    intent.getStringExtra(MolidoVpnService.EXTRA_DETAIL),
                    intent.getIntExtra(MolidoVpnService.EXTRA_PROGRESS, -1),
                )
                MolidoVpnService.STATUS_STARTING -> showStarting()
                MolidoVpnService.STATUS_SCANNING -> showScanning()
                ...
                MolidoVpnService.STATUS_CONNECTED -> beginVerification()
                MolidoVpnService.STATUS_FAILED -> {
                    ...
                    // is shown normally.
                    if (!advanceAutoScan()) {
                        showFailure(intent.getStringExtra(MolidoVpnService.EXTRA_DETAIL))
                    }
                }
                MolidoVpnService.STATUS_DISCONNECTED -> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!AppAppearance.isNight(this)) setTheme(R.style.Theme_Molido_Light)
        ...
        footerWave = OrbitFooterWave(this, palette, Strings.t("SECURED BY MolidoVPN"))
    ...
    override fun onStart() {
        super.onStart()
        val filter = IntentFilter(MolidoVpnService.ACTION_STATUS)
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
    ...
    private fun showOpeningOverlay() {
        ...
        val logo = ImageView(this).apply {
            setImageResource(R.drawable.molido_splash_logo)
    ...
    private fun failFakeConnection() {
        ...
        startService(Intent(this, MolidoVpnService::class.java)
            .setAction(MolidoVpnService.ACTION_DISCONNECT))
    ...
    private fun refreshPublicIp(resetRetry: Boolean = true) {
        ...
        if (coreExitIp.isBlank() && isTunnelActive()) {
            MolidoVpnService.lastMeasuredExitIp()
    ...
    private fun updateNotificationHealth(ip: String? = null, ping: String? = null, country: String? = null) {
        ...
        startService(Intent(this, MolidoVpnService::class.java)
            .setAction(MolidoVpnService.ACTION_NOTIFICATION_HEALTH)
            .apply {
                ip?.let { putExtra(MolidoVpnService.EXTRA_NOTIFICATION_IP, it) }
                ping?.let { putExtra(MolidoVpnService.EXTRA_NOTIFICATION_PING, it) }
                country?.let { putExtra(MolidoVpnService.EXTRA_NOTIFICATION_COUNTRY, it) }
            })
    }
    ...
    private fun onCountryKnown(country: String) {
        ...
        if (!isTunnelActive()) {
            getSharedPreferences("settings", MODE_PRIVATE).edit()
                .putString(MolidoVpnService.PREF_HOME_COUNTRY, code).apply()
            return
        }
    ...
    private fun copyFullLog() {
        Thread({
            ...
            runOnUiThread {
                ...
                clipboard?.setPrimaryClip(ClipData.newPlainText(Strings.t("MolidoVPN log"), text))
    ...
    private fun shareFullLog() {
        Thread({
            val result = runCatching {
                ...
                val target = File(dir, "molidovpn-log.txt")
                    ...
                    startActivity(
                        Intent.createChooser(
                            Intent(Intent.ACTION_SEND)
                                .setType("text/plain")
                                .putExtra(Intent.EXTRA_SUBJECT, "MolidoVPN log")
    ...
    /** Appropriate Legal Notices (AGPL-3.0 §5(d), §13). Keep. */
    private fun showLicensesSheet() {
        val dialog = Dialog(this).apply { requestWindowFeature(Window.FEATURE_NO_TITLE) }
        val sheet = LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL
            setPadding(dp(24), dp(24), dp(24), dp(24))
        ...
        sheet.addView(LinearLayout(this).apply {
            gravity = Gravity.CENTER_VERTICAL
            addView(createHeaderBackButton { dialog.dismiss() }, LinearLayout.LayoutParams(dp(48), dp(48)))
            addView(label(Strings.t("Open-source licenses"), 22f, INK, TypefaceStyle.MEDIUM))
        })
        sheet.addView(label(Strings.t("LICENSE_NOTICE"), 14f, MUTED), LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { topMargin = dp(8); bottomMargin = dp(16) })
        sheet.addView(navRow(Strings.t("Source code (AGPL-3.0)"), iconRes = R.drawable.ic_github) {
            openLink("https://github.com/hidooch980/molidovpn-android")
        }, LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT,
        ))
        sheet.addView(navRow(Strings.t("License text"), "GNU AGPL-3.0") {
            openLink("https://www.gnu.org/licenses/agpl-3.0.html")
        }, LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { topMargin = dp(8) })
    ...
    private fun openScannerScreen(animate: Boolean = true) {
        ...
        content.addView(label(Strings.t("Choose the core's endpoint-discovery budget and address families"), 14f, MUTED), LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
    ...
    private fun openSettingsScreen(animate: Boolean = true) {
        ...
        val autoReconnectRow = createToggleRow(
            ...
            preferences().edit().putBoolean(MolidoVpnService.AUTO_RECONNECT_PREF, it).apply()
        }
        ...
        val lanBypassRow = createToggleRow(
            ...
            preferences().edit().putBoolean(MolidoVpnService.LAN_BYPASS_PREF, it).apply()
        }
        ...
        // AGPL-3.0 §5(d)/§13: the interactive UI must show the licence and offer the
        // corresponding source. Do not remove this row.
        content.addView(navRow(Strings.t("Open-source licenses"), "AGPL-3.0") {
            showLicensesSheet()
        }, LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { topMargin = dp(8) })
        content.addView(label("MolidoVPN ${appVersion()}", 11.5f, Sculpt.withAlpha(MUTED, 0.7f)).apply {
            gravity = Gravity.CENTER_HORIZONTAL
    ...
    private fun chooseTunnelType() {
        ...
        showChoiceSheet(
            ...
            subtitle = Strings.t("How MolidoVPN carries your traffic"),
    ...
    private fun editAdvancedObfuscation() {
        ...
        showTextSettingsSheet(
            ...
            Strings.t("WireGuard only. Jc/Jmin/Jmax tune junk packets; I1/I2 use CPS packet patterns."),
            listOf(
    ...
    private fun openTrafficMonitorScreen() {
        ...
        content.addView(label(Strings.t("Traffic carried by MolidoVPN. Per-app attribution is not available from encrypted tunnel counters."), 14f, MUTED), LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { leftMargin = dp(4); bottomMargin = dp(24) })
    ...
    private fun openSplitTunnelScreen() {
        ...
        content.addView(header)
        content.addView(label(Strings.t("Choose which apps use MolidoVPN. Changes apply next connection."), 14f, MUTED), LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
    ...
    private fun toggleTunnel() {
        ...
        if (TunnelStatus.isActive() || visualState == OrbitDialView.State.CONNECTING) {
            ...
            startService(Intent(this, MolidoVpnService::class.java).setAction(MolidoVpnService.ACTION_DISCONNECT))
    ...
    private fun connect(config: String) {
        ...
        // scan is running.
        armAutoScanWatchdog()
        startForegroundService(Intent(this, MolidoVpnService::class.java)
            .setAction(MolidoVpnService.ACTION_CONNECT)
            .putExtra(MolidoVpnService.EXTRA_CONFIG, config))
    }
    ...
    private fun startSessionTimer(restored: Boolean) {
        val serviceStart = MolidoVpnService.connectedSinceElapsed()
    ...
    private fun autoReconnectEnabled(): Boolean = preferences().getBoolean(
        MolidoVpnService.AUTO_RECONNECT_PREF,
        MolidoVpnService.AUTO_RECONNECT_DEFAULT,
    )
     ...
     * tunnel is up. [MolidoVpnService.lanBypassEnabled] is still the only
    ...
    private fun lanBypassEnabled(): Boolean = preferences().getBoolean(
        MolidoVpnService.LAN_BYPASS_PREF,
    ...
    private fun splitTunnelSummary(): String {
        ...
        return when (settings.mode()) {
            SplitTunnelSettings.Mode.ALL -> Strings.t("All apps use MolidoVPN")
    ...
    private enum class Protocol(
        ...
        // transport (MolidoVpnService auto selection) and connects with the best.
