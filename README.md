...
تونل کل دستگاه با `VpnService` و یک اینترفیس TUN ساخته می‌شود، یعنی همهٔ اپ‌ها — نه فقط مرورگر — بدون هیچ تنظیم اضافه‌ای پوشش می‌گیرند. دیتاگرام‌های UDP و QUIC هم در فضای کاربر پل می‌شوند، پس ویدیو و بازی و تماس صوتی هم از تونل رد می‌شوند.

### 🔌 حالت‌های اتصال

- **سرورهای V2Ray** — با زیرپروتکل‌های VLESS، VMess، Trojan، Shadowsocks، Reality، XHTTP، Hysteria2، TUIC و AnyTLS
- **شارد (SHARD)** — عبور از لبه‌های CDN با MASQUE روی HTTP/3
- **WARP / WireGuard** — اتصال مستقیم به شبکهٔ کلادفلر
- **وارپ در وارپ** — لایه‌گذاری WARP برای شبکه‌هایی که خود WARP را هم فیلتر کرده‌اند
- **AmneziaWG داخلی** — بدون نیاز به وارد کردن کانفیگ جداگانه
- **سایفون (Psiphon)** — و سایفون روی وارپ برای شبکه‌های سخت‌گیرتر
- **تور (Tor)** — و تور روی وارپ (داخل تونل MASQUE) برای شبکه‌ای که خود Tor را بسته
- **V2Ray روی سایفون** — لایه‌گذاری V2Ray از داخل تونل سایفون
- **فقط DNS + DNS گیمینگ** — حالت سبک برای وقتی فقط resolver باید عوض شود
- **حالت خودکار** — یک دکمه؛ انتخاب گیت‌وی، مذاکره و بازیابی همه خودکار انجام می‌شود

### 🧠 هوشمندی

- **تست همهٔ حالت‌ها** — اسکن خودکار حالت‌های موجود برای پیدا کردن بهترین گزینه روی شبکهٔ فعلی
- **پرهیز از خروجی ایران** — گیت‌وی‌هایی که خروجی‌شان داخل ایران است کنار گذاشته می‌شوند
- **بهترین حالت به تفکیک اپراتور** — از روی گزارش‌های ناشناس کاربران امتیازدهی و رتبه‌بندی می‌شود
- **اشتراک IP تمیز کلادفلر** — کاربران با اجازهٔ خودشان بهترین آی‌پی‌های تمیز شارد را به‌صورت ناشناس به بقیهٔ کاربران همان اپراتور می‌رسانند
- **انتخاب کشور خروج** — ۲۷ کشور برای Tor و ۲۵ کشور برای Psiphon، با تعداد واقعی رله و سرور هر کدام و **تأیید واقعی اتصال**؛ دایره تا وقتی ترافیک واقعاً رد نشده باشد موفقیت اعلام نمی‌کند
- **اسکنر پس‌زمینه** — به‌صورت دوره‌ای وضعیت گیت‌وی‌ها را زیر نظر دارد
- **ضدیخ‌زدگی و جابه‌جایی خودکار (Anti-freeze / Watchdog)** — اگر یک نود شارد بی‌حرکت بماند، بدون افتادن تونل به نود دیگری سوییچ می‌شود

### ⭐ تجربه کاربری

- حالت **ساده / پیشرفته** برای خانه
- **راهنمای شروع (Onboarding)** برای کاربر تازه‌وارد
- **نوار اطلاعیه** برای پیام‌های مدیر
- **کارت تازه‌های نسخه** بعد از هر به‌روزرسانی
- **اشتراک با دوستان** با کد QR
- **کانفیگ‌های من** — وارد کردن با چسباندن متن، فایل، تصویر QR، لینک اشتراک یا Outline؛ گزینهٔ «سریع‌ترین کانفیگ من»
- کنترل از راه دور: خاموش/روشن کردن حالت‌ها و اطلاعیه‌های مدیر از طریق Worker
- رابط فارسی / انگلیسی / چینی، ویجت صفحهٔ اصلی، کاشی Quick Settings
- **تونل تفکیکی (Split tunnel)** — انتخاب اپ‌هایی که داخل یا بیرون تونل بمانند
- **Smart Split** — سایت‌های فقط SNI-بلاک‌شده (مثل یوتیوب، اینستاگرام، واتس‌اپ) مستقیم و با فرگمنت TLS رد می‌شوند تا سریع‌تر باشند؛ فقط سایت‌های تحریمی و تلگرام از نود عبور می‌کنند
- **عبور مستقیم ایران (IR direct)** برای سایت‌های داخلی
- تست سرعت، اتصال زمان‌بندی‌شده، پشتیبان‌گیری از تنظیمات، اشتراک در LAN، به‌روزرسانی خودکار
- نمایش زندهٔ وضعیت: IP خروجی با پرچم کشور، مصرف داده، مدت اتصال و لاگ لحظه‌ای
- **قطع‌کن اضطراری (Kill switch)** — اگر تونل بیفتد، شبکه کامل قطع می‌شود تا ترافیک لو نرود
- **اجبار DNS** — فقط resolver عمومی استفاده می‌شود و DNS اپراتور کامل کنار گذاشته می‌شود

### 🔒 حریم خصوصی

- گزارش‌های کیفیت اتصال کاملاً **اختیاری و پیش‌فرض خاموش** هستند
- گزارش‌ها فقط شامل شناسهٔ ناشناس نود، موفقیت/شکست، تأخیر اتصال، نوع شبکه، پلتفرم و نسخهٔ برنامه‌اند
- هیچ IP و هیچ نام کاربری ذخیره یا ارسال نمی‌شود
...
| سرویس `MolidoVpnService` | چرخهٔ حیات VPN، ساخت TUN، سرپرستی ترابری |
| سرویس `MolidoTileService` | کاشی تنظیمات سریع |
...
This is a modified version of an AGPL-3.0 open-source project; the complete corresponding source code for this app is available at [this repository](https://github.com/hidooch980/molidovpn-android), distributed under the same license ([LICENSE](LICENSE)).
